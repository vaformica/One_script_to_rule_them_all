...
def build_qc_bundle(outputs,rid,analysis,pngs,track_pdf=None):
 ...
 if track_pdf and track_pdf.exists():
  dst=bundle/f'{rid}_tracks.pdf'; shutil.copy2(track_pdf,dst); copied.append(dst.name)
 # Keep source PNGs too: useful for quick inspection and downstream reports.
...
def collect(run_dir,project_root,metadata):
 qc=project_root/'QC'; previews=qc/'track_previews'; pdfs=qc/'track_pdfs'
 # Create every shared QC destination before any copy operation.  Earlier
 # versions created only QC/, which caused the first PDF collection to fail.
 for directory in (qc, previews, pdfs, qc/'ba_track_pngs'):
 ...
 pngs=canonical_track_pngs(outputs); preview_rel='';pdf_rel=''; track_pdf=None
 if pngs:
 ...
 if pngs:
  track_pdf=pdfs/f'{rid}_tracks.pdf'
  # Prefer the analysis-generated multipage PDF for both BA and fights. It
  # preserves vector text and includes the x-y-time 3-D page.
  generated_pdfs=sorted(
   p for p in outputs.rglob('*_tracks.pdf')
   if 'QC_review_bundle' not in p.parts and p.name != f'{rid}_tracks.pdf'
  )
  if generated_pdfs:
   shutil.copy2(generated_pdfs[0],track_pdf)
  else:
   pdf_from_pngs(pngs,track_pdf)
  if track_pdf.exists():
   pdf_rel=f'track_pdfs/{track_pdf.name}'
   shutil.copy2(track_pdf,outputs/f'{rid}_tracks.pdf')
  if metadata.analysis_type=='ba':
 ...
 bundle=build_qc_bundle(outputs,rid,metadata.analysis_type,pngs,track_pdf)
 ...
 return {'status':status,'record_id':rid,'track_maps':len(pngs),'track_pdf':pdf_rel,'qc_bundle':str(bundle)}
def main():
