...
def plot_tracks(
    ...
    """Make BA-style QC maps for two-beetle combat tracks.
    ...
    draw_one(ax, list(range(len(xys))), "Combat tracks: both beetles")
    ...
    for i in range(len(xys)):
        ...
        fig, ax = plt.subplots(figsize=(8, 8))
        draw_one(ax, [i], f"Combat track: animal_{i}")
        fig.tight_layout(rect=[0, 0.10, 1, 1])
...
def process_session(args: argparse.Namespace) -> Tuple[Path, Path]:
    ...
    if n_animals < 2:
        raise ValueError(f"Combat script requires at least two animals; found {n_animals} in {traj_path}")
    start = max(0, int(args.analysis_start_frame))
    end = n_frames_total if args.window_frames is None else min(n_frames_total, start + int(args.window_frames))
    if start >= end:
        raise ValueError(f"Analysis window is empty: start={start}, end={end}, total_frames={n_frames_total}")
    arr = arr_full[start:end, :, :]

    session_json = Path(args.session_json).expanduser().resolve() if args.session_json else find_nearby_file(traj_path, "session.json")
    attributes_json = Path(args.attributes_json).expanduser().resolve() if args.attributes_json else find_nearby_file(traj_path, "attributes.json")
    session_data = read_json_file(session_json)
    attributes_data = read_json_file(attributes_json)
    default_session_name = traj_path.parent.parent.name if traj_path.parent.name == "trajectories" else traj_path.parent.name
    session_name = args.prefix or session_data.get("name") or session_data.get("session_name") or default_session_name
    prefix = safe_stem(args.prefix or session_name or traj_path.stem)
    output_dir = Path(args.output_dir).expanduser().resolve()
    output_dir.mkdir(parents=True, exist_ok=True)

    toml_path = Path(args.roi_toml).expanduser().resolve() if args.roi_toml else None
    if toml_path is None:
        matches = list(traj_path.parent.glob("*.toml")) + list(traj_path.parent.parent.glob("*.toml"))
    ...
    flat_session.update(match_metadata(metadata, str(session_name), traj_path, None))
    ...
    for animal_idx in animal_indices:
        ...
        ind_meta.update(match_metadata(metadata, str(session_name), traj_path, animal_idx))
        summary, per_frame, turtling_events, xy, tmask, imask, onset_idx = analyze_individual(
            arr[:, animal_idx, :], animal_idx, start, n_animals, rois, ind_meta, args
        )
        summary["combat_role"] = "animal0" if animal_idx == animal_indices[0] else "animal1"
        summaries.append(summary)
        per_frame.to_csv(output_dir / f"{prefix}_animal{animal_idx}_per_frame_kinematics.csv", index=False)
        turtling_events.to_csv(output_dir / f"{prefix}_turtling_events_animal{animal_idx}.csv", index=False)
        xys.append(xy)
        turt_masks.append(tmask)
        interp_masks.append(imask)
        onset_indices.append(onset_idx)

    pair_summary, pair_frame, events_df, fight_df = pairwise_metrics(xys[0], xys[1], start, rois, args)
    ...
    individual_summary_path = output_dir / f"{prefix}_combat_individual_summary.csv"
    ...
    inq_path = output_dir / f"{prefix}_InqScribe_{int(args.contact_px)}px_{args.fps:.0f}fps.txt"
    ...
    if not events_df.empty:
        ...
        pd.DataFrame(columns=["Start Time","End Time","Title","Comment"]).to_csv(inq_path, sep="\t", index=False)
    plot_tracks(xys, turt_masks, interp_masks, onset_indices, rois, png_path, pdf_path, args)
    build_data_dictionary(output_dir / f"{prefix}_summary_data_dictionary.csv")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Post-process one two-beetle IDtracker.ai combat session.")
    ...
    p.add_argument("--output-dir", default="combat_analysis", help="Output folder.")
